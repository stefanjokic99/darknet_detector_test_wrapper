...
class DetectionWithClass:
    def __init__(self, det, best_class):
        self.det = det
...
def load_network(config_file, data_file, weights):
    ...
    returns:
        ...
        class_colors
    """
    network = load_net_custom(config_file.encode("ascii"), weights.encode("ascii"), c_int(0), c_int(1))

    metadata = load_meta(data_file.encode("ascii"))
    class_names = [metadata.names[i].decode("ascii") for i in range(metadata.classes)]
    colors = class_colors(class_names)

    return network, class_names, colors, metadata
...
def detect_image(network, class_names, resized_image, original_dimension, thresh=.25, hier_thresh=.5, nms=.45):
    """
    ...
    Args:
        ...
        class_names: List of class names.
        resized_image: Input image (Resized image to network dimension).
        original_dimension: Original dimension of image.
    ...
    predict(network, resized_image.data)
    detections = get_network_boxes(network, original_dimension.width, original_dimension.height,
                                   thresh, hier_thresh, None, 1, pnum, 0)
    num = pnum[0]
    if nms:
        do_nms_sort(detections, num, len(class_names), nms)

    predictions = calculate_detections(original_dimension, detections, num, thresh, class_names, len(class_names))
    free_detections(detections, num)
    return sorted(predictions, key=lambda x: x[1])


def calculate_detections(original_dimension, dets, num, thresh, names, classes):
    selected_detections = get_actual_detections(dets, num, thresh)
    selected_detections_num = len(selected_detections)
    detections_list = []

    selected_detections.sort(key=lambda x: x.det.bbox.x)
    for i in range(selected_detections_num):
        best_class = selected_detections[i].best_class
        confidence = selected_detections[i].det.prob[best_class] * 100
        bbox = selected_detections[i].det.bbox
        bbox_info = relative_bbox2point(bbox, original_dimension)
        detections_list.append((names[best_class], confidence, bbox_info))

        for j in range(classes):
            if selected_detections[i].det.prob[j] > thresh and j != best_class:
                confidence = selected_detections[i].det.prob[j] * 100
                bbox_info = relative_bbox2point(bbox, original_dimension)
                detections_list.append((names[j], confidence, bbox_info))

    return detections_list

def get_actual_detections(dets, dets_num, thresh):
    result_arr = []
    ...
    for i in range(dets_num):
        ...
        if dets[i].best_class_idx == -1:
            continue

        for j in range(dets[i].classes):
            if dets[i].prob[j] > best_class_prob:
                best_class = j
                best_class_prob = dets[i].prob[j]

        if best_class >= 0:
            result_arr.append(DetectionWithClass(dets[i], best_class))

    return result_arr

def relative_bbox2point(bbox, original_dimension):
    return (
        round((bbox.x - bbox.w / 2) * original_dimension.width),
        round((bbox.y - bbox.h / 2) * original_dimension.height),
        round(bbox.w * original_dimension.width),
        round(bbox.h * original_dimension.height)
    )

def convert_cv2_image2darknet_image(image):
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    height, width, channels = image_rgb.shape
    darknet_image = make_image(width, height, 3)
    copy_image_from_bytes(darknet_image, image_rgb.tobytes())

    return darknet_image

# Platform-specific library path and initialization
if os.name == "posix":
    libpath = "/usr/local/lib/libdarknet.so"
elif os.name == "nt":
...
load_weights = lib.load_weights
load_weights.argtypes = [c_void_p, c_char_p]
load_weights.restype = c_void_p

fuse_conv_batchnorm = lib.fuse_conv_batchnorm
fuse_conv_batchnorm.argtypes = [c_void_p]
fuse_conv_batchnorm.restype = c_void_p

calculate_binary_weights = lib.calculate_binary_weights
calculate_binary_weights.argtypes = [c_void_p]
calculate_binary_weights.restype = c_void_p

test_detector = lib.test_detector
test_detector.argtypes = [c_char_p, c_char_p, c_char_p, c_char_p, c_float, c_float, c_int, c_int, c_int, c_char_p, c_int, c_int]
test_detector.restype = None

# draw_detections_v3 = lib.draw_detections_v3
# draw_detections_v3.argtypes = [IMAGE, POINTER(DETECTION), c_int, c_float, POINTER(c_char_p), POINTER(POINTER(IMAGE)), c_int, c_int]
# draw_detections_v3.restype = None
